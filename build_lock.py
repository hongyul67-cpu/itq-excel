# 교재 기출 세트(sets/*.js) -> 암호화(sets.enc) + 출력형태 그림(img/*.jpg -> enc/*.enc)
#
...
#
# ⚠️ 이 도구는 「📚 기출 세트」 탭 하나만 잠근다.
#    배우기·연습·실습 세트·종합시험·오답노트는 앞 세션이 직접 쓴 것이라 공개로 둔다.
#
# 암호가 두 종류인 이유 (수업용):
...
#   ⚠️ secret.json 은 고치지 말 것 — 고치면 도구 50개의 학생 코드가 전부 바뀐다.
#
# 주의: 평문 sets/ · img/ 는 .gitignore 에 있다. 절대 커밋하지 말 것.
...
OUT = os.path.join(HERE, "sets.enc")
ITER = 200_000


def extract_json():
    """sets_book.js 를 node 로 평가해 회차 배열 JSON 으로 만든다."""
    driver = ("const sets = require('./sets_book.js');\n"
              "process.stdout.write(JSON.stringify({v:1, sets:sets}));\n")
...
def main():
    ...
    ap.add_argument("--pw", help="교사용 암호. 생략하면 _weekly/secret.json 의 teacher_pw 를 쓴다")
    a = ap.parse_args()
    ...
    if not a.pw:
        ...
        if not a.pw:
            raise SystemExit("교사용 암호가 없습니다 - _weekly/secret.json 에 teacher_pw 를 넣으세요")
    ...
    sets = data["sets"]
    if not sets:
        raise SystemExit("sets/ 가 비어 있습니다 - 전사부터 하세요")

    # 배점합이 500 이 아니면 어딘가 빠뜨린 것이다. 여기서 막는다.
    for s in sets:
        tot = sum(t.get("pt", 0) for t in s["tasks"])
        if tot != 500:
            raise SystemExit("배점합이 500 이 아닙니다: %s = %d점" % (s["id"], tot))
    ...
    # 3) 출력형태 그림도 같은 내용키(CK)로 잠근다. img/*.jpg -> enc/*.jpg.enc
    #
    #    ⚠️ 캐시 함정 — 다시 빌드하면 CK 가 새로 생기는데, 브라우저가 옛 .enc 를
    #    캐시에서 꺼내 쓰면 복호화가 조용히 실패해 그림이 빈 칸으로만 보인다.
    #    그래서 빌드마다 표식(build)을 sets.enc 에 넣고, 화면은 ?b=<표식> 을 붙여 받는다.
    ...
    # 화면이 참조하는 그림이 실제로 잠겼는지 본다. 빠지면 수업 중에 빈 칸이 된다.
    have = set(os.listdir(out_dir))
    miss = sorted({t["img"].split("/")[-1] for s in sets for t in s["tasks"] if t.get("img")}
                  - {f[:-4] for f in have})
    if miss:
        raise SystemExit("그림이 img/ 에 없습니다: " + ", ".join(miss))

    io.open(OUT, "w", encoding="utf-8").write(json.dumps({
        "v": 1, "cipher": "AES-GCM", "gz": True,
        "rounds": len(sets), "imgs": nimg, "build": build_id,
    ...
    # 화면 쪽에도 표식을 박는다. 안 하면 옛 index.html 이 캐시에서 나온다.
    ...
    nmo = sum(1 for s in sets if s["kind"] == "모의고사")
    print("  회차 %d개 (모의 %d · 기출 %d) · 원본 %dKB -> gzip %dKB -> sets.enc %dKB"
          % (len(sets), nmo, len(sets) - nmo, len(raw) // 1024, len(gz) // 1024,
             os.path.getsize(OUT) // 1024))
    print("  출력형태 그림 %d장 -> enc/*.enc  (빌드 표식 %s)" % (nimg, build_id))
    print("")
